...
class _VolumeSState extends State<VolumeS> {
  ...
  void updateAnswer() {
    if (_given == "Radius" && _pi == "22/7") {
      setState(() {
        answer = (22 * 4) * pow((int.parse(input.text)), 3) / (7 * 3);
      });
    } else if (_given == "Radius" && _pi == "π") {
      setState(() {
        answer = (3.14 * 4) * pow(int.parse(input.text), 3) / 3;
      });
    } else if (_given == "Volume" && _pi == "22/7") {
      setState(() {
        answer =
            pow((3 * int.parse(input.text)) / (4 * (22 / 7)), 1 / 3).toDouble();
      });
    } else if (_given == "Volume" && _pi == "π") {
      setState(() {
        answer = pow((3 * int.parse(input.text)) / (4 * (3.14159)), 1 / 3)
            .toDouble();
      });
    }
  }
