...
class SurfaceAreaS extends StatefulWidget {
  const SurfaceAreaS({super.key});

  @override
  State<SurfaceAreaS> createState() => _SurfaceAreaSState();
}

class _SurfaceAreaSState extends State<SurfaceAreaS> {
  String _pi = "22/7";
  ...
  void updateAnswer() {
    if (_given == "Radius" && _pi == "22/7") {
      setState(() {
        answer = 4 * (22 / 7) * pow(int.parse(input.text), 2).toDouble();
      });
    } else if (_given == "Radius" && _pi == "π") {
      setState(() {
        answer = 4 * (3.14159) * pow(int.parse(input.text), 2).toDouble();
      });
    } else if (_given == "Surface Area" && _pi == "22/7") {
      setState(() {
        answer = pow(int.parse(input.text) / (4 * (22 / 7)), 1 / 2).toDouble();
      });
    } else if (_given == "Surface Area" && _pi == "π") {
      setState(() {
        answer = pow(int.parse(input.text) / (4 * (3.14159)), 1 / 2).toDouble();
      });
    }
  }
}
