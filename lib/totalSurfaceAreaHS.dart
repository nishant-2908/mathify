...
class _SurfaceAreaHSState extends State<SurfaceAreaHS> {
  ...
  void updateAnswer() {
    if (_given == "Radius" && _pi == "22/7") {
      setState(() {
        answer = 3 * (22 / 7) * pow(int.parse(input.text), 2).toDouble();
      });
    } else if (_given == "Radius" && _pi == "π") {
      setState(() {
        answer = 3 * (3.14159) * pow(int.parse(input.text), 2).toDouble();
      });
    } else if (_given == "Surface Area" && _pi == "22/7") {
      setState(() {
        answer = pow(int.parse(input.text) / (3 * (22 / 7)), 1 / 2).toDouble();
      });
    } else if (_given == "Surface Area" && _pi == "π") {
      setState(() {
        answer = pow(int.parse(input.text) / (3 * (3.14159)), 1 / 2).toDouble();
      });
    }
  }
}
